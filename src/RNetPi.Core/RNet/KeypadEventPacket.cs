using System;
using System.Collections.Generic;

namespace RNetPi.Core.RNet;

/// <summary>
/// Represents a keypad event packet
/// </summary>
public class KeypadEventPacket : EventPacket
{
    // Valid keypad event keys from the JavaScript implementation
    public static readonly HashSet<ushort> KEYS = new()
    {
        
    };

    public KeypadEventPacket()
    {
        MessageType = 0x05;
    }

    public byte GetControllerID()
    {
        return SourceControllerID;
    }

    public byte GetZoneID()
    {
        return SourceZoneID;
    }

    public byte GetKeypadID()
    {
        return SourceKeypadID;
    }

    public ushort GetKeyID()
    {
        return EventID;
    }

    public bool IsValidKey()
    {
        return KEYS.Contains(EventID);
    }

    /// <summary>
    /// Creates a KeypadEventPacket from an EventPacket
    /// </summary>
    public static KeypadEventPacket FromPacket(EventPacket eventPacket)
    {
        if (eventPacket.MessageType != 0x05)
        {
            throw new ArgumentException("Cannot create KeypadEventPacket from packet with MessageType != 0x05");
        }

        var keypadEventPacket = new KeypadEventPacket();
        eventPacket.CopyToPacket(keypadEventPacket);
        return keypadEventPacket;
    }
}